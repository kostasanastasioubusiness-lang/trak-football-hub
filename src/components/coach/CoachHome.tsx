    ...
    <div className="app-container flex flex-col min-h-screen">
      <div className="flex-1 px-[18px] pt-4 pb-24">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="font-heading text-2xl font-black tracking-wider text-foreground">Coach Dashboard</h1>
        ...
        {/* Quick Actions */}
        <p className="font-heading text-[17px] font-bold tracking-wider text-foreground mb-3">QUICK ACTIONS</p>
        <div className="grid grid-cols-3 gap-2 mb-5">
        ...
        {/* Recent Assessments */}
        <p className="font-heading text-[17px] font-bold tracking-wider text-foreground mb-3">RECENT ASSESSMENTS</p>
        {recentAssessments.length === 0 ? (
          ...
          <div className="space-y-2">
            {recentAssessments.map((a) => (
              <div key={a.id} className="bg-card rounded-xl border border-white/5 p-3 flex items-center justify-between"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm text-foreground font-bold">{(a.squad_players as any)?.player_name}</p>
                ...
                <div className="text-right">
                  <p className="font-heading text-xl text-coach-orange leading-none">{Number(a.coach_rating).toFixed(1)}</p>
                  <p className="text-[8px] text-muted-foreground uppercase">Coach Rating</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex justify-around items-center">
        {items.map(i => (
          <button key={i.id} onClick={() => navigate(i.path)}
            ...
            <span className={`text-[10px] font-semibold tracking-wide ${active === i.id ? 'text-coach-orange' : 'text-muted-foreground'}`}>{i.label}</span>
          </button>
        ))}
      </div>
    </nav>
